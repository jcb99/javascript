//Categories of Operators

/*1. Assignment: "="
 a = 10;
*/
/*=============================================================================

2. Arithmetic: "+", "-", "*", "/", "%" (Remainder Amt...called "Modulus")

var m = 10 % 3;
console.log(m);
*/

/*=============================================================================
3. Increment/Decrement: "++", "--"
var a = 1;
a++;
console.log(a++); //This increment operator works after the 'a' has already been
printed to the screen so it still appears as 2 until we reprint it on the next line
We could have used a ++a to then display 3 in the first console.log instead
console.log(a);
*/

/*=============================================================================
4. String: "+" (As concatenation, NOT ADDITION)

*/

/*=============================================================================
5. Precedence: "()" (You can use parens to specify order of ops)
var b = 1 + 2 * 3; //This is a completely different result than the line that follows
var c = (1 + 2) * 3; //Order of Ops PEMADS
console.log(b);
console.log(c);

*/
/*=============================================================================
6. Function Invocation Operators: "()" (You can also use parens to invocate a function)
console.log('Hello'); The parens are fucntion invocation ops...This is showing us
a function called "log"..You can call it with arguments..We are passing in a
string argument "Hello"
*/

/*=============================================================================
7. Logical Operators: "||" (or), "&&" (and)

*/
/*=============================================================================
8. Member Accessor Operator: //These is the "." in Console.Log()...It allows
us to access the different "members" of this specific object

this allows us to access the log function of the console object inside of JavaScript

*/

/*=============================================================================
9. Code Block Operators "{}"

*/

/*=============================================================================
10. Array Element Access Operator "[]"

*/
